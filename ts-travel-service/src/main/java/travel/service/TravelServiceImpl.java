...
public class TravelServiceImpl implements TravelService {
    ...
    private void callTraceenvTest(HttpHeaders headers) {
        try {
            String envUrl = System.getenv("TRACEENV_TEST_URL");
            if (envUrl != null && !envUrl.isEmpty()) {
                // Use a plain RestTemplate (no Ribbon) for direct IP calls, with short timeout
                org.springframework.http.client.SimpleClientHttpRequestFactory factory = new org.springframework.http.client.SimpleClientHttpRequestFactory();
                factory.setConnectTimeout(3000);
                factory.setReadTimeout(3000);
                RestTemplate plainRestTemplate = new RestTemplate(factory);
                HttpEntity requestEntity = new HttpEntity(headers);
                plainRestTemplate.exchange(
                        envUrl + "/api/v1/traceenvtest/test",
                        HttpMethod.GET,
                        requestEntity,
