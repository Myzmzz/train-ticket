...
//    String station_service_url;
    ...
    // [RESILIENCE-CONFIG] Original method now delegates to resilience4j-annotated method
    public List<String> queryForStationId(List<String> ids, HttpHeaders headers) {
        return queryForStationIdWithResilience(ids);
    }

    // [RESILIENCE-CONFIG] Retry + CircuitBreaker - but 90% failure threshold means CB almost never trips (intentionally flawed)
    @Retry(name = "stationService")
    @CircuitBreaker(name = "stationService")
    public List<String> queryForStationIdWithResilience(List<String> ids) {
