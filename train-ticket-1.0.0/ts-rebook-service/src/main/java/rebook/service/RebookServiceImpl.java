...
public class RebookServiceImpl implements RebookService {
    ...
    private Response<Order> getOrderByRebookInfo(RebookInfo info, HttpHeaders httpHeaders) {
        ...
        HttpHeaders newHeaders = getAuthorizationHeadersFrom(httpHeaders);
        return getOrderByRebookInfoWithRetry(requestUrl, newHeaders);
    }

    // [RESILIENCE-CONFIG] Retry only - no circuit breaker, no bulkhead, no timeout (intentionally dangerous)
    @Retry(name = "orderService")
    public Response<Order> getOrderByRebookInfoWithRetry(String requestUrl, HttpHeaders httpHeaders) {
        ...
        return reGetOrderByRebookInfo.getBody();
    }
