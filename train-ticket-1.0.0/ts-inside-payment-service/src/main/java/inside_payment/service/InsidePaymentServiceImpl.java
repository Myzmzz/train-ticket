// [RESILIENCE-CONFIG] Added for fault tolerance testing
...
public class InsidePaymentServiceImpl implements InsidePaymentService {
    ...
    public Response pay(PaymentInfo info, HttpHeaders headers) {
        ...
        ResponseEntity<Response<Order>> reGetOrderResults = callOrderServiceGetOrder(
                requestOrderURL, requestGetOrderResults, headers);
        ...
        if (result.getStatus() == 1) {
            ...
            if (totalExpand.compareTo(money) > 0) {
                ...
                ResponseEntity<Response> reOutsidePaySuccess = callPaymentServiceExchange(
                        payment_service_url + "/api/v1/paymentservice/payment",
                        requestEntityOutsidePaySuccess, headers);
    ...
    private Response setOrderStatus(String tripId, String orderId, HttpHeaders headers) {
        ...
        if (tripId.startsWith("G") || tripId.startsWith("D")) {
            ...
            String order_service_url = getServiceUrl("ts-order-service");
            ResponseEntity<Response> reModifyOrderStatusResult = callOrderServiceStatusExchange(
                    order_service_url + "/api/v1/orderservice/order/status/" + orderId + "/" + orderStatus,
                    requestEntityModifyOrderStatusResult, headers);
    ...
    // ==================== [RESILIENCE-CONFIG] Resilience4j annotated helper methods ====================

    /**
     * Calls order-service to retrieve an order, with retry, circuit breaker, and bulkhead protection.
     */
    @Retry(name = "orderService")
    @CircuitBreaker(name = "orderService", fallbackMethod = "getOrderFallback")
    @Bulkhead(name = "orderService")
    public ResponseEntity<Response<Order>> callOrderServiceGetOrder(String url, HttpEntity requestEntity, HttpHeaders headers) {
        return restTemplate.exchange(url, HttpMethod.GET, requestEntity,
                new ParameterizedTypeReference<Response<Order>>() {});
    }

    /**
     * Fallback for callOrderServiceGetOrder when the order-service is unavailable.
     */
    public ResponseEntity<Response<Order>> getOrderFallback(String url, HttpEntity requestEntity, HttpHeaders headers, Throwable t) {
        LOGGER.error("[RESILIENCE-FALLBACK][callOrderServiceGetOrder][Order service unavailable][url: {}][error: {}]", url, t.getMessage());
        Order pendingOrder = new Order();
        Response<Order> response = new Response<>(0, "Service temporarily unavailable - pending", pendingOrder);
        return ResponseEntity.ok(response);
    }

    /**
     * Calls order-service to update order status, with retry, circuit breaker, and bulkhead protection.
     */
    @Retry(name = "orderService")
    @CircuitBreaker(name = "orderService", fallbackMethod = "orderStatusFallback")
    @Bulkhead(name = "orderService")
    public ResponseEntity<Response> callOrderServiceStatusExchange(String url, HttpEntity requestEntity, HttpHeaders headers) {
        return restTemplate.exchange(url, HttpMethod.GET, requestEntity, Response.class);
    }

    /**
     * Fallback for callOrderServiceStatusExchange when the order-service is unavailable.
     */
    public ResponseEntity<Response> orderStatusFallback(String url, HttpEntity requestEntity, HttpHeaders headers, Throwable t) {
        LOGGER.error("[RESILIENCE-FALLBACK][callOrderServiceStatusExchange][Order service unavailable][url: {}][error: {}]", url, t.getMessage());
        Response response = new Response<>(0, "Order status update failed - service temporarily unavailable", null);
        return ResponseEntity.ok(response);
    }

    /**
     * Calls payment-service with retry, circuit breaker, and bulkhead protection.
     */
    @Retry(name = "paymentService")
    @CircuitBreaker(name = "paymentService", fallbackMethod = "paymentServiceFallback")
    @Bulkhead(name = "paymentService")
    public ResponseEntity<Response> callPaymentServiceExchange(String url, HttpEntity requestEntity, HttpHeaders headers) {
        return restTemplate.exchange(url, HttpMethod.POST, requestEntity, Response.class);
    }

    /**
     * Fallback for callPaymentServiceExchange when the payment-service is unavailable.
     */
    public ResponseEntity<Response> paymentServiceFallback(String url, HttpEntity requestEntity, HttpHeaders headers, Throwable t) {
        LOGGER.error("[RESILIENCE-FALLBACK][callPaymentServiceExchange][Payment service unavailable][url: {}][error: {}]", url, t.getMessage());
        Response response = new Response<>(0, "Payment service temporarily unavailable", null);
        return ResponseEntity.ok(response);
    }
