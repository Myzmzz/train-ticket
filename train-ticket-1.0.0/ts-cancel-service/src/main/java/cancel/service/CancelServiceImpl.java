...
//                    order.setStatus(OrderStatus.CANCEL.getCode());
    ...
    public boolean drawbackMoney(String money, String userId, HttpHeaders headers) {
        ...
        ResponseEntity<Response> re = callInsidePaymentDrawback(url, requestEntity, newHeaders);
    ...
    /**
     * Calls ts-inside-payment-service for drawback refund, with retry, circuit breaker, and bulkhead protection.
     */
    ...
    /**
     * Fallback for callInsidePaymentDrawback when ts-inside-payment-service is unavailable.
     */
    public ResponseEntity<Response> drawbackFallback(String url, HttpEntity requestEntity, HttpHeaders headers, Throwable t) {
        LOGGER.error("[RESILIENCE-FALLBACK][callInsidePaymentDrawback][Inside payment service unavailable][url: {}][error: {}]", url, t.getMessage());
        Response response = new Response<>(0, "Refund pending - payment service temporarily unavailable", null);
